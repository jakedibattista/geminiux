# I Built an AI That Audits Your Website — And It Was a Disaster Before It Was Great
...
We entered the [Gemini Live Agent Challenge](https://googlegemini.devpost.com/) — Track 3: UI Navigator — with a plan that looked like this on a whiteboard:
...
Tech: Google ADK + ComputerUseToolset + gemini-3.1-pro-preview
      (the newest, fanciest model for every single agent)
Timeline estimate: elegant, done by noon
```

This is what engineers call a "napkin architecture." It looks clean because you haven't tried to build it yet.

We were using **Google's Agent Development Kit (ADK)**, which is a framework for building multi-agent AI systems. The plan was to give each persona agent a `ComputerUseToolset` — essentially, the ability to control a browser like a human — and let them loose on any URL.
...
The first problem appeared the moment we tried to actually run an agent.

`gemini-2.5-computer-use-preview-10-2025` (the model we needed for browser control) threw:

```
400 INVALID_ARGUMENT: UI actions are not enabled for this project
```

Google restricts computer-use capabilities at the *enterprise project level*. Our project wasn't allowlisted. We couldn't use Vertex AI for this at all.

Fine. We pivoted to Google AI Studio instead — same models, different API key. This worked! Until we tried to add our custom `log_issue` function alongside the browser tool. The ADK threw a new error:

```
400 INVALID_ARGUMENT: Tool use is not supported with this model configuration
```

The ADK strictly prohibits combining `ComputerUseToolset` with *any* custom function tools in specialized models. You can have browser control or custom tools. Not both. Our entire architecture required both.

### Collapse #2: The Rate Limit Bloodbath

We found a workaround: split each persona into two separate ADK agents. One for browsing, one for reporting. It was clunky but theoretically functional.

We ran three personas simultaneously.
...
By this point, we'd rearchitected twice. On our third attempt, something subtler went wrong: ADK was *swallowing our screenshots*.

The multimodal state passing — the mechanism that's supposed to attach screenshot images to findings — worked differently than the documentation suggested. We couldn't reliably attach visual evidence to individual findings. The screenshots existed. They just weren't making it to where they needed to go.

Three collapses. One conclusion:

> **We dropped the ADK entirely.**

---

## The First Rebuild: Native Everything

With the ADK out of the picture, we wrote our own execution loop from scratch using Python's `asyncio` and the standard Google GenAI SDK. No framework. Just direct API calls, raw Playwright browser control, and an `asyncio.gather()` to run everything in parallel.
...
Timeline estimate: overconfident but less so
```
...
Then we ran it against a real website.
...
Here's what happened when we audited a real app:
...
We had built a tool that was filing bug reports about its own limitations.
...
We wrote this to an AI. The AI started listening to it. The false positives dropped dramatically. The product became dramatically more trustworthy.
...
We tried fixing the math. Introduced separate X and Y scale factors:
...
We fixed the `isMobile` detection, which was computing device type by checking whether the agent's *name* contained the word "mobile." (This is the kind of heuristic that works exactly once and breaks every time thereafter.) Still wrong.

We spent a meaningful portion of a hackathon trying to get CSS circles to land in the right places on screenshots.

The root cause, which we eventually discovered: **Gemini internally resamples images before processing them.** When the model reports `x=200, y=400`, those are coordinates in the model's internal image space — not the original Playwright viewport. There is no documented way to know what resolution Gemini uses internally. The coordinates are accurate to a reality we can't access.
...
By this point we were on our third architecture and had made peace with our reduced scope. But a new problem appeared during heavy testing: **agents were silently freezing**.

Terminal logs would show `Sending state to model...` and then nothing. No error. No timeout. Just silence, forever, until we killed the process manually.

The fix was obvious in retrospect: async API calls with no timeouts will wait forever if the network drops. We wrapped everything in `asyncio.wait_for()` with a 180-second guardrail.
...
But the crashes revealed what was actually causing the stalls: **rate limits**. The parallel persona agents were exhausting their model quotas in the first minute of any multi-persona audit. Every model we tried hit a different wall:
...
The browser agents are now on `gemini-2.5-flash`. It's not the flashiest name on the spec sheet, but it doesn't explode when you give it screenshots and ask for parallel execution.
...
Started with Google's ADK framework. Hit walls with model restrictions, rate limits, and broken state passing. Threw it out and wrote our own execution loop with native Python `asyncio` and raw Playwright. Less magic, more control.

**Pivot 2 — Browsing Agents to Crawl-and-Review:**  
Even with native Playwright, asking an AI to *simultaneously* navigate a browser, reason about UX, and log findings was too much. It was like asking someone to drive, read a map, and write a book report at the same time. We split the jobs: a dedicated crawler does all the driving and screenshot-taking first. Then separate reviewer agents look at the photos and write the report. Much better.

**Pivot 3 — Static Report to Interactive Presentation:**  
The original output was a text report. It was useful, but it didn't feel like something you'd want to share. We added a presentation layer: a full slide deck with per-slide audio narration, grounded in real screenshots from the audit. Every slide shows the actual product, not AI-generated placeholder art.

---

## What It Actually Does Today

Paste any URL. Pick from built-in personas — First-Time Visitor, Mobile User, Accessibility User — or describe your own:

> *"A 45-year-old project manager at a mid-sized logistics company evaluating our B2B product for the first time."*

Hit Generate. Gemini creates a full persona card. The persona gets added to your selection.
...
2. A vision model reviews every screenshot for quality — rejecting loading spinners, blank frames, and broken embeds before any persona sees them.
...
I want to be honest about something: this was built in close collaboration with AI coding assistants. Cursor. Claude. Gemini itself. The architecture decisions, the debugging sessions, the "we need to completely rethink this" moments — all of it happened in conversation with AI.

There's something a little recursive about that. We built an AI that audits websites, using AI, while debugging AI models, for a competition run by the company that makes the AI models we were using.

The handoff documents in this codebase — `AGENT_HANDOFF.md`, the 37-entry hackathon diary — were written specifically so that a *new* AI coding session could pick up the context where the previous one left off. Context windows run out. When you're deep in debugging why Gemini's internal image coordinates don't map to Playwright viewport coordinates, you don't want to re-explain the entire architecture from scratch at the start of the next session.

So we built documentation designed to be read by AI.

The docs became part of the system. Architecture decisions, bugs found and fixed, scope changes, the reasoning behind every pivot — all logged in a format that let the next session continue without losing momentum.
...
The hackathon submission is in. Whether it places or not, the thing we built is real, works on real websites, and tells you something genuinely useful that you didn't know before you ran it.
...
The three pivots taught me more about building production AI systems than any amount of reading would have. You learn where the framework boundaries are by hitting them. You learn what a model can't do by watching it fail confidently. You learn what your product actually *is* by deciding what it isn't.

If you want to audit your site: [AuditMySite](https://auditmysite.vercel.app) *(link live pending hackathon submission review).*

If you want to look at the code, the full 37-entry build diary, or the architecture docs: they're all in the repo.

And if your AI agents are filing bug reports about things that aren't broken — welcome. We've been there. The fix is to tell them, in the most explicit possible language, that some things are simply not their problem.

Sometimes that's good advice for everyone.
...
*Total architectural pivots: 3. Red circles drawn on screenshots: 0 (removed). Audits that produced a score of 15/100 for a working website: 1 (early days, fixed now).*
