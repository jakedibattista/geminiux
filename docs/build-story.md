...
**Presentation slides were showing tall stitched screenshots instead of normal-sized images**
After the screenshot-grounding fix, the presentation became more truthful but also uglier: many slide cards were rendering the full stitched composite screenshot, which shrank into a very thin unreadable strip inside the slide layout. Root cause: the crawler only stored one page-level artifact for the post-audit pipeline, and it was the stitched composite. The presentation layer had no cleaner image to choose. Fix: `crawler.py` now uploads both artifacts per page/device: the stitched `composite_*` PNG and a first-frame `viewport_*` PNG. The viewport image is stored as `desktop_presentation_screenshot` / `mobile_presentation_screenshot`, and `audit_recap.py` now maps composite evidence URLs back to those cleaner preview shots when assigning slide visuals. Result: the Screenshots tab still uses the tall composite for evidence fidelity, but the founder presentation now shows a single normal-sized screenshot that fits the slide much better.
...
**Silent presentation failure due to TTS rate limits**  
Google's text-to-speech preview model (`gemini-2.5-flash-tts`) imposes a strict limit of 100 requests per day per project on the free tier. Because the presentation builder generates unique audio for every slide (typically 4–6 slides per audit), a few concurrent test runs quickly exhausted the daily quota. When the `429 RESOURCE_EXHAUSTED` error was thrown, it crashed the entire presentation-generation step, leaving the audit permanently stalled without a slide deck. Fix: Wrapped the audio generation call in `audit_recap.py` with a specific `try/except` block. When the TTS quota is exhausted, the backend now catches the error, logs a warning, and continues building the visual slide deck. The presentation still renders perfectly in the frontend, just without voiceover audio, allowing audits to succeed even under rate limits.
